...
def extract_text_from_file(file_content: bytes, filename: str) -> str:
    """PDF, DOCX 파일에서 텍스트를 추출하고, 실패 시 이미지 분석을 위한 Base64 데이터 존재 플래그 반환"""
    ...
    if file_extension == ".pdf":
        try:
            ...
            # 텍스트 추출 실패 시, Base64 데이터로 전환하도록 유도
            else:
                return "BASE64_CONVERSION_NEEDED" 
                
        except Exception:
            # PDF 파싱 중 오류 발생 시, Base64 데이터로 전환하도록 유도
            return "BASE64_CONVERSION_NEEDED"
...
def create_multimodal_parts(file_content: bytes, file_mime_type: str, extracted_text: str) -> List[Any]:
    ...
    # 1. Base64 이미지 처리 (PDF 텍스트 추출 실패 시)
    if extracted_text == "BASE64_CONVERSION_NEEDED":
        try:
            # Base64 인코딩된 데이터를 parts에 직접 추가
            encoded_content = base64.b64encode(file_content).decode("utf-8")
            parts.append({
                "inlineData": {
                    "data": encoded_content,
                    "mimeType": "image/jpeg" # PDF를 이미지로 간주하고 시각 분석을 강제하기 위해 JPEG mimeType 사용
                }
            })
            parts.append("경고: PDF 텍스트 추출에 실패했습니다. Base64 인코딩된 파일을 **고해상도 이미지처럼 분석하여** 시각적 내용(그래프, 그림)을 파악하고 보고서에 통합하십시오.")
        except Exception:
            parts.append("PDF Base64 인코딩 실패. 분석 가능한 데이터가 없습니다.")
            
    
    # 2. 일반 이미지 파일 처리 (JPEG, PNG 등)
    ...
    # 3. 텍스트 파트 (성공적으로 추출된 텍스트)
    if extracted_text and extracted_text != "BASE64_CONVERSION_NEEDED":
...
def determine_routing(target_tag: str) -> Tuple[str, str]:
    ...
    if "분석" in tag:
        # 논리 분석 및 구조화에 강한 모델 (PDF 분석 강제를 위해 Pro 모델 지정)
        return "gemini-2.5-pro", "당신은 세계적인 분석가입니다. 제공된 문서 및 시각 데이터를 **최우선으로 분석**하여 보고서 초안을 작성하고, 명확한 근거와 시사점을 제시하십시오. 만약 Base64 PDF 데이터가 있다면 반드시 그 시각적 내용을 분석하십시오."
