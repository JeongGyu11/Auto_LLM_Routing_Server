...
def determine_routing(target_tag: str):
    ...
    if target_tag == "분석":
        ...
        system_prompt = "당신은 최고의 금융 분석가입니다. 입력된 데이터를 기반으로 논리적인 구조의 심층 분석 보고서 초안을 작성하십시오. 핵심 수치를 명확히 제시하십시오. **분석할 내용이 부족하더라도, 입력된 정보와 요청을 바탕으로 추론 가능한 보고서를 반드시 작성해야 합니다.**"
    
    elif target_tag == "문체":
        # 문체 다듬기 및 효율성이 중요한 작업은 Flash 모델 (Claude 역할)
        model_name = "gemini-2.5-flash"
        system_prompt = "당신은 문장력을 최고로 다듬는 전문 편집자입니다. 입력된 텍스트를 전문적이고 자연스러운 문체로 다듬고, 핵심만 간결하게 요약하십시오. **분석 대신 편집에 집중하십시오.**"
    
    else:
        # Controller 태그가 잘못된 경우, 명확히 오류를 반환하도록 수정 (Controller 사용 강제)
        raise HTTPException(status_code=400, detail="Controller 라우팅 태그는 '분석' 또는 '문체' 중 하나여야 합니다.")
...
def extract_text_from_file(file_content: bytes, filename: str) -> str:
    ...
    if filename.lower().endswith('.pdf'):
        try:
            ...
            # 텍스트 추출 시도. 실패하면 LLM이 이미지 분석에 집중하도록 유도하는 메시지 반환
            text = "".join(page.extract_text() for page in reader.pages if page.extract_text())
            return text if text.strip() else "[텍스트 추출 실패: PDF 파일 내용이 없습니다. 이미지만으로 분석을 시도합니다.]"
        except Exception:
        ...
        try:
            ...
            return text if text.strip() else "DOCX 파일에서 텍스트를 추출하지 못했습니다."
        except Exception:
...
async def run_multimodal_pipeline(file_content: bytes, file_name: str, file_mime: str, user_request: str, target_tag: str):
    """
    Pipeline 역할: 텍스트 추출 → 이미지 객체 준비 → Controller 판단 → 최종 LLM 실행까지의 흐름을 정의합니다.
    """
    
    # 5-1. Step 1: 문서 텍스트 추출
    text_data = extract_text_from_file(file_content, file_name)
    image_data = None
    
    # 5-2. Step 2: 이미지 객체 준비 (이미지 파일만)
    is_image_file = file_mime.startswith('image/')
    
    if is_image_file:
        # 이미지 파일인 경우에만 PIL Image 객체로 변환 시도
        try:
    ...
    # 이미지 데이터가 있으면 Vision 모델에 전달 (멀티 모달)
    if image_data:
        contents_list.append(image_data)
        
    # 텍스트 데이터가 있고, 지원되지 않는 파일 형식이 아닐 경우 프롬프트에 추가
    if text_data and not text_data.startswith("지원되는 문서"):
        contents_list.append(f"[문서 텍스트]: {text_data}")
    
    # 텍스트도 이미지도 없으면 오류 발생
    if not contents_list:
        raise HTTPException(status_code=400, detail="파일에서 추출할 수 있는 내용(텍스트 또는 이미지)이 없습니다. 지원 파일 형식: PNG, JPG, DOCX, TXT, 텍스트가 포함된 PDF.")


    # 5-3. Step 3: Controller 판단 및 LLM 선택
    # *************************************************************************
    # IMPORTANT: Controller 함수 내에서 target_tag가 유효하지 않으면 HTTP 400 발생
    # *************************************************************************
    ...
    # 5-4. Step 4: 최종 LLM 실행 (Controller 지시에 따름)
    try:
